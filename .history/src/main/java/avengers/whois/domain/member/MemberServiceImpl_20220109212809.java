...
public class MemberServiceImpl implements MemberService, UserDetailsService {
    ...
    private void storeFiles(WorkerMemberDto workerMemberDto, WorkerMember data) throws IOException {
        MultipartFile a = workerMemberDto.getFname();
        if (a != null) {
        } else {

            String convertedFName = fileManager.convertAndStore(a);
            data.setFName(convertedFName);

        }

        a = workerMemberDto.getResume();

        if (a != null) {
        } else {
            String convertedResume = fileManager.convertAndStore(a);
