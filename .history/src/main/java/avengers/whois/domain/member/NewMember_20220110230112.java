...
public class NewMember {
    ...
    public NewMemberDto toDtoWOpw() {
        NewMemberDto data = NewMemberDto.builder().fname(this.fname).phoneNumber(this.phoneNumber).birthDate(this.birthDate)
                    .address(this.address).createdDate(this.createdDate).modifiedDate(this.modifiedDate).build();
        if (this.memberType == 'W') { // 당시 객체가 W타입, Worker면?
                data.setGender(this.gender);
                data.setFinding(this.finding);
            return 
                    .gender(this.gender)
                    .finding(this.finding).resume(this.resume).build();
        } else {    // 'C'타입, Corporate 면?
            return NewMemberDto.builder().fname(this.fname).name(name)
        }
