...
import avengers.whois.domain.file.MultiFile;
...
public class NewMember {
    ...
    @Column(nullable = true) // 요즘 무서운 시대라 성별체크는 nullable 해야하나 생각 들기 시작함
    private char gender;
    ...
    // 대표자명+설립일+사업자번호 > 기업 정상여부확인 후 정보 리턴 //TODO: vit

    // 로그인정보(session)에 포함되지 않는 기본정보 로딩해갈 Dto변환함수
    public NewMemberDto toDtoWOpw() {
        NewMemberDto data = NewMemberDto.builder().fname(this.fname).phoneNumber(this.phoneNumber)
                .birthDate(this.birthDate)
                .address(this.address).createdDate(this.createdDate).modifiedDate(this.modifiedDate).build();
        if (this.memberType == 'W') {
            // 당시 객체가 W타입, Worker면 개인회원에 해당하는 정보 세팅
            data.setGender(this.gender);
            data.setFinding(this.finding);
            data.setResume(this.resume);
        } else {
            // 'C'타입, Corporate 면 기업회원에 해당하는 정보 세팅
            data.setCorpNo(this.corpNo);
            data.setCorpName(this.corpName);
            data.setIndustryField(this.industryField);
        }
        return data;
    }
